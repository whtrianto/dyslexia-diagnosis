...
<h1 class="h3 mb-0">
    Data Disleksia
    <button class="btn btn-info btn-sm border-0 float-right" type="button" data-toggle="modal" data-target="#Tambahtb_penyakit">Tambah</button>
</h1>
<hr>
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
    ...
    <tbody>
        <?php
        $no = 1;
        $data_penyakit = mysqli_query($kon, "SELECT * FROM tb_penyakit ORDER BY id ASC");
...
<?php
// Proses Tambah
if (isset($_POST['Tambahtb_penyakit'])) {
    ...
    $cek = mysqli_num_rows(mysqli_query($kon, "SELECT * FROM tb_penyakit WHERE penyakit='$penyakit'"));
    if ($cek > 0) {
        echo '<script>alert("penyakit sudah ada");history.go(-1);</script>';
    } else {
        $insert = mysqli_query($kon, "INSERT INTO tb_penyakit (penyakit, definisi, penanganan, link) VALUES ('$penyakit','$definisi','$penanganan','$link')");
...
if (isset($_POST['SimpanEdit'])) {
    ...
    $cek = mysqli_query($kon, "SELECT * FROM tb_penyakit WHERE penyakit='$penyakit' AND id!='$id'");
    if (mysqli_num_rows($cek) > 0) {
        echo '<script>alert("Kode sudah digunakan!");history.go(-1);</script>';
    } else {
        $update = mysqli_query($kon, "UPDATE tb_penyakit SET penyakit='$penyakit', definisi='$definisi', penanganan='$penanganan', link='$link' WHERE id='$id'")
...
if (!empty($_GET['hapus'])) {
    $id = $_GET['hapus'];
    $hapus = mysqli_query($kon, "DELETE FROM tb_penyakit WHERE id='$id'");
    echo $hapus ? '<script>window.location.href="editd.php";</script>' : '<script>alert("Gagal Hapus Data");</script>';
}
?>

<!-- Modal Tambah -->
<div class="modal fade" id="Tambahtb_penyakit" tabindex="-1" role="dialog">
        ...
        <div class="modal-content border-0">
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="Tambahtb_penyakit" class="btn btn-success">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<?php
$data_penyakit_modal = mysqli_query($kon, "SELECT * FROM tb_penyakit ORDER BY id ASC");
