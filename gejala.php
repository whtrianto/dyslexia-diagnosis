...
<h1 class="h3 mb-0">
    Gejala Disleksia
    <button class="btn btn-info btn-sm border-0 float-right" type="button" data-toggle="modal" data-target="#Tambahtb_gejala">Tambah Gejala</button>
</h1>
<hr>
<table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
    ...
    <tbody>
        <?php
        $no = 1;
        $data_gejala = mysqli_query($kon, "SELECT * FROM tb_gejala ORDER BY id ASC");
...
<?php
// Proses Tambah
if (isset($_POST['Tambahtb_gejala'])) {
    $kode = htmlspecialchars($_POST['Tambah_Kode_tb_gejala']);
    $gejala = htmlspecialchars($_POST['Tambah_gejala']);

    $cek = mysqli_num_rows(mysqli_query($kon, "SELECT * FROM tb_gejala WHERE kode='$kode'"));
    if ($cek > 0) {
        echo '<script>alert("Kode sudah ada");history.go(-1);</script>';
    } else {
        $insert = mysqli_query($kon, "INSERT INTO tb_gejala (kode, gejala) VALUES ('$kode', '$gejala')");
...
if (isset($_POST['SimpanEdit'])) {
    ...
    $kode = htmlspecialchars($_POST['Edit_Kode_tb_gejala']);
    $gejala = htmlspecialchars($_POST['Edit_gejala']);

    $cek = mysqli_query($kon, "SELECT * FROM tb_gejala WHERE kode='$kode' AND id!='$id'");
    if (mysqli_num_rows($cek) > 0) {
        echo '<script>alert("Kode sudah digunakan!");history.go(-1);</script>';
    } else {
        $update = mysqli_query($kon, "UPDATE tb_gejala SET kode='$kode', gejala='$gejala' WHERE id='$id'")
...
if (!empty($_GET['hapus'])) {
    $id = $_GET['hapus'];
    $hapus = mysqli_query($kon, "DELETE FROM tb_gejala WHERE id='$id'");
    echo $hapus ? '<script>window.location.href="gejala.php";</script>' : '<script>alert("Gagal Hapus Data");</script>';
}
?>

<!-- Modal Tambah -->
<div class="modal fade" id="Tambahtb_gejala" tabindex="-1" role="dialog">
    <div class="modal-dialog" role="document">
        <div class="modal-content border-0">
            <?php
            $getKode = mysqli_query($kon, "SELECT MAX(kode) AS maxID FROM tb_gejala");
            ...
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" name="Tambah_Kode_tb_gejala" value="<?= $kodeBaru; ?>" class="form-control" readonly>
                    </div>
                    <div class="form-group">
                        <label>Gejala :</label>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" name="Tambahtb_gejala" class="btn btn-success">Simpan</button>
                </div>
            </form>
        </div>
    </div>
...
<?php
$data_gejala_modal = mysqli_query($kon, "SELECT * FROM tb_gejala ORDER BY id ASC");
            ...
            <div class="modal-content border-0">
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label>Kode :</label>
                            <input type="text" name="Edit_Kode_tb_gejala" value="<?= $d['kode']; ?>" class="form-control" required>
                        </div>
                        <div class="form-group">
                            <label>Gejala :</label>
